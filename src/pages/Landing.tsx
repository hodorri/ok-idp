...
export default function Landing() {
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-xs sm:text-sm">OK금융그룹 IDP 신청</span>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/login">
              <Button variant="ghost" size="sm" className="text-xs hidden sm:inline-flex">로그인</Button>
            </Link>
            <Link to="/login">
      ...
      {/* Hero */}
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-10 sm:pt-16 pb-10 sm:pb-12">
        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-12">
          <div className="flex-1 space-y-4 sm:space-y-5 text-center md:text-left">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold leading-tight tracking-tight text-foreground">
              자격증 취득 비용,<br />
          ...
          <div className="flex-shrink-0 animate-float">
            <img src={certiMan} alt="자격증 취득 캐릭터" className="w-48 sm:w-64 md:w-80 drop-shadow-lg" />
          </div>
        </div>
      </section>
